...
{
    const sf::Vector2f rotation_Radius = {20.f, 40.f};
    const sf::Vector2f delta_R = mousePosition - eye_R.ellipseR.getPosition();
    const sf::Vector2f delta_L = mousePosition - eye_L.ellipseL.getPosition();
    float angle_R = atan2(delta_R.y, delta_R.x);
    float angle_L = atan2(delta_L.y, delta_L.x);
    float insideEye_R = inside(mousePosition, eye_R.ellipseR.getPosition(),
                               rotation_Radius.x, rotation_Radius.y);
    float insideEye_L = inside(mousePosition, eye_L.ellipseL.getPosition(),
                               rotation_Radius.x, rotation_Radius.y);
    float R = std::pow(rotation_Radius.x * std::cos(angle_R), 2) + std::pow(rotation_Radius.y * std::sin(angle_R), 2);
    float L = std::pow(rotation_Radius.x * std::cos(angle_L), 2) + std::pow(rotation_Radius.y * std::sin(angle_L), 2);
    if (insideEye_R > R)
    {
    ...
    {
        eye_R.zra4okR.setPosition(mousePosition);
    }
    if (insideEye_L > L)
    {
    ...
    {
        eye_L.zra4okL.setPosition(mousePosition);
    }
}
